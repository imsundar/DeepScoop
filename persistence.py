...
def load_kb_vector_store(cluster_id):
    """
    Load a specific vector store based on the provided cluster ID.
    """
    global kb_vector_store
    ...
    try:
        kb_vector_store[cluster_id] = FAISS.load_local(faiss_file, bedrock_embeddings, allow_dangerous_deserialization=True)
        # vector_stores[cluster_id] = vector_store
        print(f"Loaded kb vector store for component: {cluster_id}.")
        return kb_vector_store[cluster_id]
    except Exception as e:
        print(f"Failed to load kb vector store for {cluster_id}: {str(e)}")
        return None
